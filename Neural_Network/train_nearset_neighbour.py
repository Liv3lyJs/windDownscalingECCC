...
def print_gpu_memory_usage():
    ...
    gpu_memories = result.stdout.decode('utf-8').strip().split('\n')
    for i, gpu_memory in enumerate(gpu_memories):
        print(f'GPU {i} memory usage: {gpu_memory} MB')

def data_generator(input_data, topo_data, skip_data, label_data, batch_size):
    num_samples = len(input_data)
    while True:
        for offset in range(0, num_samples, batch_size):
            batch_inputs = input_data[offset:offset + batch_size]
            batch_topo = topo_data[offset:offset + batch_size]
            batch_skip = skip_data[offset:offset + batch_size]
            batch_labels = label_data[offset:offset + batch_size]
            
            if len(batch_inputs) != batch_size:  # Skip the last batch if it's not full
                continue

            yield (batch_inputs, batch_topo, batch_skip), batch_labels
...
def path_creation(INPUT_FILE, LABEL_FILE, TOPO_FILE, PATH_RESULTS, HP_NAME, DATA_ALREADY_SPLIT):
    ...
    HP_RESULT_PATH = f"{PATH_RESULTS}/nearest_neighbour_interpolation_qcnb_{var_name}/{HP_NAME}_HP_Search"
    HP_DIR = f"{HP_NAME}_{var_name}_hp_search"

    if DATA_ALREADY_SPLIT:
        #Input file
        PATH_INPUT_TRAIN = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_INPUT_VALID = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_INPUT_TEST = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        ...
        #label file
        PATH_LABEL_TRAIN = '/home/jfg000/ss5/data_superResolution/label/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_LABEL_VALID = '/home/jfg000/ss5/data_superResolution/label/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_LABEL_TEST = '/home/jfg000/ss5/data_superResolution/label/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        ...
        PATH_SKIP_TRAIN = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_SKIP_VALID = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_SKIP_TEST = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        ...
        #Topo file
        PATH_TOPO = '/home/jfg000/ss5/data_superResolution/topography/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/ME_MG_Z0_data'
...
def load_data_already_split(INPUT_FILE_TRAIN, INPUT_FILE_VALID, INPUT_FILE_TEST, LABEL_FILE_TRAIN, LABEL_FILE_VALID, LABEL_FILE_TEST, SKIP_FILE_TRAIN, SKIP_FILE_VALID, SKIP_FILE_TEST, TOPO_FILE):
    ...
    # print(f'The shape of the skip_train connection for training is: {skip_train.shape}')
    # print(f'The shape of the skip_valid connection for training is: {skip_valid.shape}')
    # print(f'The shape of the skip_test connection for training is: {skip_test.shape}')
...
def dataProcessing_already_split(inputs_train, inputs_valid, inputs_test, labels_train, label_valid, label_test, skip_train, skip_valid, skip_test, topo_train):
    ...
    print(f'The input train shape is: {inputs_train.shape}')
    print(f'data processing inputs')
    print_memory_usage()
    ...
    print(f'The labels_train shape is: {labels_train.shape}')
    print(f'data processing lables')
    print_memory_usage()

    skip_train = skip_train.transpose((0,2,3,1)) 
    skip_valid = skip_valid.transpose((0,2,3,1)) 
    skip_test = skip_test.transpose((0,2,3,1)) 
    print(f'The skip_train shape is: {skip_train.shape}')
    print(f'data processing skip')
    print_memory_usage()
...
def tuner_initialization(hyper_parameters_search, NUM_TRIALS, EXECUTIONS_PER_TRIAL, directory, HP_DIR):
    ...
    tuner = BayesianOptimization(                            # RandomSearch
        ...
        project_name='nearest_neighboor_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000001_hp_search_Final_search'   #HP_DIR
    )
...
def hyper_parameters_search(hp):
    ...
    drop_rate = hp.Choice('drop_rate', values=[0.25]) #, 0.05, 0.10, 0.15, 0.2     0.10, 0.20, 0.30, 0.40, 0.50
    #Define the alpha value for the leakyRelu activation function
    alpha = hp.Choice('alpha', values=[0.25]) #0.01, 0.1, 0.2, 0.3
    ...
    model = mod.deepRU_article_interpolation_combine_loss(units, activation, lr, kernel_size, kernel_size_e1, kernel_size_e2, kernel_size_e3, kernel_size_d1, kernel_size_d2, kernel_size_d3,
...
def tuner_search(train_dataset, steps_per_epoch, EPOCH_TRY, val_dataset, validation_steps, earlystop, reduceLROnPlateau, endWhenNaN, VERBOSE):
    """
...
def model_train(model, train_dataset, steps_per_epoch, BATCH_SIZE, EPOCHS, VERBOSE, val_dataset, validation_steps, best_model_weights, earlystop, reduceLROnPlateau, endWhenNaN):
    """
...
def de_standardize(test_data_input, predictions, test_data_label, skip_test_data):
    std_inputs = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_inputs = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy') 
    std_label = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_label = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy') 
    std_skip = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_skip = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy')
...
if __name__ == '__main__':
    ...
    input_train, input_val, inputs_test, label_train, label_val, label_test, skip_train, skip_valid, skip_test, topo_train = load_data_already_split(INPUT_FILE_TRAIN, INPUT_FILE_VALID, INPUT_FILE_TEST, LABEL_FILE_TRAIN, LABEL_FILE_VALID, LABEL_FILE_TEST, SKIP_FILE_TRAIN, SKIP_FILE_VALID, SKIP_FILE_TEST, TOPO_FILE)
    print_memory_usage()
    print(f'The shape for the input is: {input_train.shape}')
    print(f'The shape for the skip connection is: {skip_train.shape}')
    print(f'The shape for the topography is: {topo_train.shape}')
    print(f'The training set size is: {input_train.shape}')
    print(f'The validation set size is: {input_val.shape}')
    print(f'The test set size is: {inputs_test.shape}')

    # Data processing
    input_train, input_val, input_test, label_train, label_val, label_test, skip_train, skip_valid, skip_test, topo_train, topo_val, topo_test = dataProcessing_already_split(input_train, input_val, inputs_test, label_train, label_val, label_test, skip_train, skip_valid, skip_test, topo_train)


    def shuffled_data_generator(input_data, topo_data, skip_data, label_data, batch_size):
        data_size = len(input_data)
        while True:
            # Shuffle indices
            indices = np.random.permutation(data_size)
            
            # Yield batches using shuffled indices
            for i in range(0, data_size, batch_size):
                batch_indices = indices[i:i+batch_size]
                batch_input = input_data[batch_indices]
                batch_topo = topo_data[batch_indices]
                batch_skip = skip_data[batch_indices]
                batch_label = label_data[batch_indices]
                
                yield ((batch_input, batch_topo, batch_skip), batch_label)

    batch_size = 32

    # Create datasets using generators
    train_dataset = tf.data.Dataset.from_generator(
        lambda: shuffled_data_generator(input_train, topo_train, skip_train, label_train, batch_size),
        output_signature=(
            (tf.TensorSpec(shape=(None, 64, 64, 26), dtype=tf.float32),
    ...
    # Validation dataset (no need to shuffle)
    val_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(input_val, topo_val, skip_valid, label_val, batch_size),
        output_signature=(
            (tf.TensorSpec(shape=(None, 64, 64, 26), dtype=tf.float32),
            tf.TensorSpec(shape=(None, 64, 64, 3), dtype=tf.float32),
            tf.TensorSpec(shape=(None, 48, 48, 1), dtype=tf.float32)),
    ...
    print(f'Memory used after tuner initialization ')
    print_memory_usage()
    print('\n')
    ...
    print(f'Memory used after replacing defaut run_trial method')
    print_memory_usage()
    print('\n')
    ...
    print(f'Memory used after call back initialization')
    print_memory_usage()
    print('\n')

    #Start the hyper-parameter search and get the best model. Keras doc here: https://keras.io/keras_tuner/ 
    tuner = tuner_search(train_dataset, steps_per_epoch, EPOCH_TRY, val_dataset, validation_steps, earlystop, reduceLROnPlateau, endWhenNaN, VERBOSE)
    print(f'Memory used after tuner search')
    print_memory_usage()
    print('\n')
    ...
    for j, hyp in enumerate(best_hp, start=1):
        ...
        model = mod.deepRU_article_interpolation_combine_loss(units=hyp.get("units"), activation=hyp.get("activation"), lr=hyp.get("lr"), kernel_size=hyp.get("kernel_size"), kernel_size_e1=hyp.get("kernel_size_e1"), kernel_size_e2=hyp.get("kernel_size_e2"),  kernel_size_e3=hyp.get("kernel_size_e3"),
        ...
        history = model_train(model, train_dataset, steps_per_epoch, hyp.get("batch_size"), int(1.2*EPOCH_TRY), VERBOSE, val_dataset, validation_steps, best_model_weights, earlystop, reduceLROnPlateau, endWhenNaN)
