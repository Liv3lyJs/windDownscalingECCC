...
def path_creation(INPUT_FILE, LABEL_FILE, TOPO_FILE, PATH_RESULTS, HP_NAME, DATA_ALREADY_SPLIT):
    ...
    HP_RESULT_PATH = f"{PATH_RESULTS}/non_interpolation_qcnb_{var_name}/{HP_NAME}_HP_Search" 
    HP_DIR = f"{HP_NAME}_{var_name}_hp_search"

    if DATA_ALREADY_SPLIT:
        #Input file
        PATH_INPUT_TRAIN = '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'    #/home/jfg000/ss5/Data/input/domaine/east_canada_test_sequential_train/UU_VV_TT_P0_PN_H_CX_SD_WGE_data
        PATH_INPUT_VALID = '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_INPUT_TEST = '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        INPUT_FILE_TRAIN = os.path.join(PATH_INPUT_TRAIN, 'normalize.npy')   
        INPUT_FILE_VALID = os.path.join(PATH_INPUT_VALID, 'normalize.npy')   
        INPUT_FILE_TEST = os.path.join(PATH_INPUT_TEST, 'normalize.npy')   

        #label file
        PATH_LABEL_TRAIN = '/home/jfg000/ss5/data_superResolution/label/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_LABEL_VALID = '/home/jfg000/ss5/data_superResolution/label/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_LABEL_TEST = '/home/jfg000/ss5/data_superResolution/label/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        LABEL_FILE_TRAIN = os.path.join(PATH_LABEL_TRAIN, 'normalize.npy') 
        LABEL_FILE_VALID = os.path.join(PATH_LABEL_VALID, 'normalize.npy') 
        LABEL_FILE_TEST = os.path.join(PATH_LABEL_TEST, 'normalize.npy') 

        #Skip connection file 
        PATH_SKIP_TRAIN = '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_SKIP_VALID = '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_valid_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        PATH_SKIP_TEST =  '/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_test_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data'
        SKIP_FILE_TRAIN = os.path.join(PATH_SKIP_TRAIN, 'normalize.npy') 
        SKIP_FILE_VALID = os.path.join(PATH_SKIP_VALID, 'normalize.npy') 
        SKIP_FILE_TEST = os.path.join(PATH_SKIP_TEST, 'normalize.npy') 

        #Topo file
        PATH_TOPO = '/home/jfg000/ss5/data_superResolution/topography/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/ME_MG_Z0_data'
        TOPO_FILE = os.path.join(PATH_TOPO, 'normalize.npy') #normalize.npy
        ...
        #Path where are located the input file.
        if TEMPERATURE:
            INPUT_FILE = os.path.join(INPUT_FILE, 'rdps.npy')
            LABEL_FILE = os.path.join(LABEL_FILE, 'caldas.npy')
            var_name = 'TT'
        else:
            INPUT_FILE = os.path.join(INPUT_FILE, 'normalize.npy')
            LABEL_FILE = os.path.join(LABEL_FILE, 'normalize.npy')
            TOPO_FILE = os.path.join(TOPO_FILE, 'normalize.npy')

        return INPUT_FILE, LABEL_FILE, TOPO_FILE, directory, HP_RESULT_PATH, HP_DIR, var_name

def load_data(INPUT_FILE, LABEL_FILE, TOPO_FILE, DATA_ALREADY_SPLIT):
    """
    Loading the input and label data used to train the model. For set that are not already splitted. 
    """
    inputs = np.load(INPUT_FILE)
    labels = np.load(LABEL_FILE)
    topo = np.load(TOPO_FILE)

    return inputs, labels, topo
...
def load_data_already_split(INPUT_FILE_TRAIN, INPUT_FILE_VALID, INPUT_FILE_TEST, LABEL_FILE_TRAIN, LABEL_FILE_VALID, LABEL_FILE_TEST, SKIP_FILE_TRAIN, SKIP_FILE_VALID, SKIP_FILE_TEST, TOPO_FILE):
    ...
    print(f'The size for the validation is: {inputs_valid.shape}')
    inputs_test = np.load(INPUT_FILE_TEST)
    print(f'The size fo the test is: {inputs_test.shape}')
    ...
    # topo_train_reshaped = np.tile(topo, (int(inputs_train.shape[0] / 5), 1, 1, 1))
    # print(f'The topography shape is : {topo_train_reshaped.shape}')

    #Show the images
    
    # for image in range(20):
    #     print(f'The image number is : {image}')
    #     fig, axes = plt.subplots(1, 4, figsize=(20, 5))  # Adjusted figsize for better visualization

    #     axes[0].imshow(inputs_train[image, 0])
    #     axes[0].set_title('input')
    #     axes[0].axis('off')

    #     axes[1].imshow(labels_train[image, 0])
    #     axes[1].set_title('target')
    #     axes[1].axis('off')

    #     axes[2].imshow(skip_train[image, 0])
    #     axes[2].set_title('skip')
    #     axes[2].axis('off')

    #     axes[3].imshow(topo_train_reshaped[image, :, :, 0])
    #     axes[3].set_title('topo')
    #     axes[3].axis('off')

    #     plt.show()
    #     plt.close(fig)

    #Show the momory usage: 
    print_memory_usage()
...
def dataProcessing(inputs, labels, topo, TRAINING_SIZE, VALID_SIZE, TEST_SIZE):
    """
    Reshaping the inputs and labels data in a channel last form to allow the network to be able to apply the neural network(NN) on the data 
    efficiently [sample, width, height, channel]. Once the input datas of the network are reshaped, it is possible to split them into
    the training, validation and testing split for the inputs and labels data. 
    INPUT
    :Param inputs: Input data for the NN. 
    :Param labels: Label of the input data for the NN. 
    RETURN
    ...
    inputs_reshaped = inputs.transpose((0,2,3,1))
    topo_reshaped = topo.transpose((0,2,3,1))
    labels_reshaped = labels.transpose((0,2,3,1))

    topo_reshaped = np.repeat(topo_reshaped, 4619, axis=0)
    
    # Converting the numpy array to a tensor of float32 (See book p.449 yellow section).
    inputs_reshaped_tensor = tf.convert_to_tensor(inputs_reshaped, dtype='float32')
    topo_reshaped_tensor = tf.convert_to_tensor(topo_reshaped, dtype='float32')
    labels_reshaped_tensor = tf.convert_to_tensor(labels_reshaped, dtype='float32')

    # Splitting the data (train, validation, test)
    # inputs
    input_train = inputs_reshaped_tensor[:TRAINING_SIZE]
    topo_train = topo_reshaped_tensor[:TRAINING_SIZE]

    input_val = inputs_reshaped_tensor[TRAINING_SIZE:(TRAINING_SIZE+VALID_SIZE)]
    topo_val = topo_reshaped_tensor[TRAINING_SIZE:(TRAINING_SIZE+VALID_SIZE)]

    input_test = inputs_reshaped_tensor[(TRAINING_SIZE+VALID_SIZE):]
    topo_test = topo_reshaped_tensor[(TRAINING_SIZE+VALID_SIZE):]

    # labels
    label_train = labels_reshaped_tensor[:TRAINING_SIZE]
    label_val = labels_reshaped_tensor[TRAINING_SIZE:(TRAINING_SIZE+VALID_SIZE)]
    label_test = labels_reshaped_tensor[(TRAINING_SIZE+VALID_SIZE):]

    return input_train, input_val, input_test, label_train, label_val, label_test, topo_train, topo_val, topo_test
...
def dataProcessing_already_split(inputs_train, inputs_valid, inputs_test, labels_train, label_valid, label_test, skip_train, skip_valid, skip_test, topo_train):
    ...
    # # IMPORTANT
    # # IMPORTANT
    # # Deleting channels This will change depending of the initial data.
    # channels_to_delete = [4, 5, 10, 11, 16, 17, 22, 23]
    # inputs_train = np.delete(inputs_train, channels_to_delete, axis=3)
    # inputs_valid = np.delete(inputs_valid, channels_to_delete, axis=3)
    # inputs_test = np.delete(inputs_test, channels_to_delete, axis=3)
    ...
    topo_train_reshaped = np.tile(topo, (int(inputs_train.shape[0] / 5), 1, 1, 1))
    # for top in topo_train_reshaped:
    #     plt.imshow(top[:,:,2])
    #     plt.show() 
    topo_val_reshaped = np.tile(topo, (int(inputs_valid.shape[0] / 5), 1, 1, 1))
    # for top in topo_val_reshaped:
    #     plt.imshow(top[:,:,2])
    #     plt.show() 
    topo_test_reshaped = np.tile(topo, (int(inputs_test.shape[0] / 5), 1, 1, 1))
    # for top in topo_train_reshaped:
    #     plt.imshow(top[:,:,2])
    #     plt.show() 
...
def tuner_initialization(hyper_parameters_search, NUM_TRIALS, EXECUTIONS_PER_TRIAL, directory, HP_DIR):
    ...
    tuner = BayesianOptimization(                            # RandomSearch
        ...
        overwrite=False,
        directory='/fs/site5/eccc/cmd/x/jfg000/CNN_results_article/hyper_parameters_search_UU',  #TODO put the name back : directory
        project_name='NoInterpol_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000001_hp_search_article_writting' #Final_search TODO ibid HP_DIR
    )
...
def hyper_parameters_search(hp):
    ...
    loss_weights = hp.Choice('loss_weights', values=[0.5])  # 0.5, 1.0, 5.0
    #Define the dropout rate for the DeepSD model
    drop_rate = hp.Choice('drop_rate', values=[0.30]) #0.25, 0.30, 0.35
    #Define the alpha value for the leakyRelu activation function
    alpha = hp.Choice('alpha', values=[0.30]) #0.25, 0.30, 0.35
 
    #Choose the model from model.py that the user want to work with.
    #TODO, the model could also be an hyper-parameter. 
    model = mod.deepRU_article_non_interpolation_combine_loss(units, activation, lr, kernel_size, kernel_size_e1, kernel_size_e2, kernel_size_e3, kernel_size_d1, kernel_size_d2, kernel_size_d3,
...
def plot_graph(history):
    ...
    plt.title("Training and validation loss MSE+SSIM")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
...
def de_standardize(test_data_input, predictions, test_data_label, skip_test_data):
    std_inputs = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_inputs = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy') 
    std_label = np.load('/home/jfg000/ss5/data_superResolution/label/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_label = np.load('/home/jfg000/ss5/data_superResolution/label/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy') 
    std_skip = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy')
    mean_skip = np.load('/home/jfg000/ss5/data_superResolution/input/domaine/non_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy')
...
def mean_metric_pred_imagecsv(HP_RESULT_PATH, j, hyp, mean_MAE, mean_MSE, mean_RMSE, mean_PSNR, mean_SSIM):
    #Create csv file header, which are image number, learning_rate, batch_size, mae, mse, rmse, psnr, ssim.
    fieldnames = ['image_number', 'learning_rate', 'batch_size', 'drop_rate', 'alpha', 'loss_weights', 'mean_mae', 'mean_mse', 'mean_rmse', 'mean_psnr', 'mean_ssim'] #, 'mean_ssim'
    #Open the csv file and append the to the file the information of the model and the metrics value between the original image and the predicted image
    with open(f'{HP_RESULT_PATH}/average_metrics_predicted_images.csv', 'a', newline='') as csvfile:
        ...
        row_dict = {
            #Write the image number that was regenerated
            'image_number': j,
            #Write hyper-parameters used
            'learning_rate': hyp.get("lr"),
            'batch_size': hyp.get("batch_size"),
            'drop_rate': hyp.get("drop_rate"),
            'alpha': hyp.get("alpha"),
            'loss_weights': hyp.get("loss_weights"),
            #Write the mae metric score
...
if __name__ == '__main__':
    ...
    if DATA_ALREADY_SPLIT:
        ...
        #     axes[0].imshow(input_train[image, :, :, 0])
        #     axes[0].set_title('input')
        #     axes[0].axis('off')

        #     axes[1].imshow(label_train[image, :, :, 0])
        #     axes[1].set_title('target')
        #     axes[1].axis('off')
        ...
        #     axes[3].imshow(topo_train[image, :, :, 0])
        #     axes[3].set_title('topo')
        #     axes[3].axis('off')
        ...
        inputs, labels, topo = load_data(INPUT_FILE, LABEL_FILE, TOPO_FILE, DATA_ALREADY_SPLIT)
        #Calling dataProcessing function that will create a train, validation and test set for the input and label data.
        input_train, input_val, input_test, label_train, label_val, label_test, topo_train, topo_val, topo_test = dataProcessing(inputs, labels, topo, TRAINING_SIZE, VALID_SIZE, TEST_SIZE)
    ...
    #If the user is not doing an hyper-parameter search, train the model with choosen values. 
        ...
        model = mod.deepRU_article_non_interpolation_combine_loss(units=UNITS, activation=ACTIVATION_FUNCTION, lr=LEARNING_RATE, kernel_size=KERNEL_SIZE, num_hidden_layer=NUM_HIDDEN_LAYERS, WIND=WIND, regL1_conv01=0.01, regL1_conv02=0.01, regL2_conv01=0.01, regL2_conv02=0.01, drop_rate=0.5, alpha=0.01, loss_weights=0.2)
    ...
    for j, hyp in enumerate(best_hp, start=1):
        ...
        model = mod.deepRU_article_non_interpolation_combine_loss(units=hyp.get("units"), activation=hyp.get("activation"), lr=hyp.get("lr"), kernel_size=hyp.get("kernel_size"), kernel_size_e1=hyp.get("kernel_size_e1"), kernel_size_e2=hyp.get("kernel_size_e2"),  kernel_size_e3=hyp.get("kernel_size_e3"),
        ...
        test_data_input = input_test
        test_topo_data = topo_test
        skip_test_data =  skip_test
        test_data_label = label_test
        ...
        for images in range(len(test_data_input_de_standardized)):
            ...
            if TEMPERATURE: 
                fig, axes = create_prediction_image(images, test_data_input_de_standardized, predictions_de_standardized, test_data_label_de_standardized, 5,
                ...
                plt.close()
            if (WIND_UU or WIND_VV or WIND_UV) and images <= 100:
                fig, axes = create_prediction_image(images, test_data_input_de_standardized, predictions_de_standardized, test_data_label_de_standardized, 0,
                ...
                #Create spectral density graph for each images
                power_spectral_density_graph_one_image(predictions_de_standardized, test_data_label_de_standardized, directory, images, test_data_skip_de_standardized)
        ...
        #Average Power spectal on the whole test set
        power_spectral_density_graph_average(predictions_de_standardized, test_data_label_de_standardized, directory, test_data_skip_de_standardized)
