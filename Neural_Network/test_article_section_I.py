...
def power_spectral_density_graph_average(predicted_images, predicted_images2, predicted_images3, ground_truth_images, directory, test_data_skip_de_standardized):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(freqs, avg_psd_pred_db2[:len(freqs)], label='Bilinear interpolation')
    plt.plot(freqs, avg_psd_pred_db3[:len(freqs)], label='Nearest Neighbour interpolation')
...
def power_spectral_density_graph_average_log(predicted_images, predicted_images2, predicted_images3, ground_truth_images, directory, test_data_skip_de_standardized):
    num_images = predicted_images.shape[0]
    print(f'The power spectral density number of images to regenerate is: {predicted_images.shape}')
    ...
    # Convert PSD to dB
    avg_psd_gt_db = avg_psd_gt_azimuthal
    avg_psd_pred_db = avg_psd_pred_azimuthal
    avg_psd_pred_db2 = avg_psd_pred_azimuthal2
    avg_psd_pred_db3 = avg_psd_pred_azimuthal3
    avg_psd_benchmark_db = avg_psd_benchmark_azimuthal
    ...
    if not os.path.exists(directory):
        os.makedirs(directory)
    plt.savefig(f'{directory}/average_psd_log.png')
    plt.close()
...
def pdf(predicted_image1, predicted_image2, predicted_image3, ground_truth_image, directory, images, test_data_skip_de_standardized):
    ...
    plt.xlabel('Wind Speed (m/sec)', fontsize=20)
...
def pdf_log(predicted_image1, predicted_image2, predicted_image3, ground_truth_image, directory, images, test_data_skip_de_standardized):
    # Flatten the data
    predict1_flatten = predicted_image1[images, :, :, 0].flatten()
    predict2_flatten = predicted_image2[images, :, :, 0].flatten()
    predict3_flatten = predicted_image3[images, :, :, 0].flatten()
    ...
    plt.savefig(f'{directory}/wind_speed_pdf_log{images+1}.png')
    plt.close()
...
def average_pdf_log(predicted_image1, predicted_image2, predicted_image3, ground_truth_image, directory, test_data_skip_de_standardized):
    ...
    # Add labels, title, and legend
    plt.xlabel('Wind Speed (m/s)', fontsize=20)
    plt.ylabel('PDF (log scale)', fontsize=20)
    #plt.title('Average PDF of Wind Speed Predictions')
    plt.legend(loc='upper right', fontsize=14)
...
def combine_graphs(predicted_image1, predicted_image2, predicted_image3, ground_truth_image, directory, test_data_skip_de_standardized):
    ...
    ax1.plot(wind_speed_range, pdf2, label='Bilinear interpolation')
    ax1.plot(wind_speed_range, pdf3, label='Nearest-Neighbour interpolation')
    ax1.plot(wind_speed_range, pdf1, label='No interpolation')
    ...
    ax2 = fig.add_axes([0.47, 0.25, 0.40, 0.35])  # Adjust these values for the inset position and size
    high_speed_filter = wind_speed_range > 15  # Filter wind speeds > 15 m/s
    ax2.plot(wind_speed_range[high_speed_filter], pdf2_log[high_speed_filter], label='Bilinear interpolation (log)')
    ax2.plot(wind_speed_range[high_speed_filter], pdf3_log[high_speed_filter], label='Nearest-Neighbour interpolation (log)')
    ax2.plot(wind_speed_range[high_speed_filter], pdf1_log[high_speed_filter], label='No interpolation (log)')
...
def average_error_location(predictions, prediction_bilinear, prediction_nearest, label, Baseline, directory):
    ...
    im1 = axes[0].imshow(average_error_pred_bi, cmap='magma', origin='lower', vmin=vmin, vmax=vmax, aspect='auto')
    axes[0].set_title(f'Bilinear interpolation:\n Average: {final_average_error_pred_bi:.2f} m/s', fontsize=20)
    cbar1 = fig.colorbar(im1, ax=axes[0], orientation='vertical') #, shrink=0.8, aspect=30
    cbar1.ax.tick_params(labelsize=18)

    im2 = axes[1].imshow(average_error_pred_near, cmap='magma', origin='lower', vmin=vmin, vmax=vmax, aspect='auto')
    axes[1].set_title(f'Nearest Neighbour interpolation:\n Average: {final_average_error_pred_near:.2f} m/s', fontsize=20)
    cbar2 = fig.colorbar(im2, ax=axes[1], orientation='vertical')
    cbar2.ax.tick_params(labelsize=18)

    im3 = axes[2].imshow(average_error_pred, cmap='magma', origin='lower', vmin=vmin, vmax=vmax, aspect='auto')
    axes[2].set_title(f'No interpolation:\n Average: {final_average_error_pred:.2f} m/s', fontsize=20)
    ...
    cbar4 = fig.colorbar(im4, ax=axes[3], orientation='vertical')
    cbar4.ax.tick_params(labelsize=18)
    
    #plt.suptitle('Difference in error for the prediction and Baseline', fontsize=24)
    axes[0].axis('off')
    axes[1].axis('off')
    axes[2].axis('off')
    axes[3].axis('off')
...
if __name__ == '__main__':
    ...
    HP_RESULT_PATH = f"/home/jfg000/ss5/CNN_results_article/articles_testing/article_section_I_{ANALYSED_DOMAIN}" 
    

    # Parameter for no interpolation: 
    lr_no_interp = 0.001           #0.001
    drop_rate_no_interp = 0.30     #0.30
    alpha_no_interp = 0.30         #0.30
    loss_weights_no_interp = 0.5   #0.5
    # Path for model weights
    path_weights_no_interp = '/home/jfg000/ss5/CNN_results_article/non_interpolation_qcnb_UU/NoInterpol_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_9_cb' # '/home/jfg000/ss5/CNN_results_article/non_interpolation_qcnb_UU/NoInterpol_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_9_cb'
    ...
    lr_bilinear = 0.001       #0.001
    drop_bilinear = 0.25      #0.25
    alpha_bilinear = 0.25     #0.25
    loss_bilinear = 1.0       #1.0
    # Path for model weights
    path_weights_bilinear = '/home/jfg000/ss5/CNN_results_article/bilinear_interpolation_qcnb_UU/bilinear_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_7_cb' #'/home/jfg000/ss5/CNN_results_article/bilinear_interpolation_qcnb_UU/bilinear_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_7_cb'
    # Input data to test the model with 
    input_file_test_bilinear = f'/home/jfg000/ss5/data_superResolution/input/domaine/bilinear_interpolation/domaine_creation/east_canada_squential_test_allpasses_{ANALYSED_DOMAIN}/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/normalize.npy'
    skip_bilinear = f'/home/jfg000/ss5/data_superResolution/input/domaine/bilinear_interpolation/domaine_creation/east_canada_squential_test_allpasses_{ANALYSED_DOMAIN}/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/normalize.npy'
    topo_bilinear = f'/home/jfg000/ss5/data_superResolution/topography/bilinear_interpolation/domaine_creation/east_canada_squential_train_allpasses_{ANALYSED_DOMAIN}/ME_MG_Z0_data/normalize.npy'
    # Std deviation and mean to de-standardize the prediction
    std_inputs_de_standardize_path_bilinear = '/home/jfg000/ss5/data_superResolution/input/domaine/bilinear_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy'
    mean_inputs_de_standardize_path_bilinear = '/home/jfg000/ss5/data_superResolution/input/domaine/bilinear_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy'
    
    # Parameter for nearest neighbour interpolation:
    lr_nearest_neighbour = 0.001    #0.001
    drop_nearest_neighbour = 0.25   #0.25
    alpha_nearest_neighbour = 0.25  #0.25
    loss_nearest_neighbour = 1.0    #1.0
    # Path for model weights
    path_weights_nearest_neighbour = '/home/jfg000/ss5/CNN_results_article/nearest_neighbour_interpolation_qcnb_UU/nearest_neighboor_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_2_cb'   #'/home/jfg000/ss5/CNN_results_article/nearest_neighbour_interpolation_qcnb_UU/nearest_neighboor_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_ME_MG_Z0_000000000000_Final_search_HP_Search/model/Searchmodel_2_cb'
    # Input data to test the model with 
    input_file_test_nearest_neighbour = f'/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/domaine_creation/east_canada_squential_test_allpasses_{ANALYSED_DOMAIN}/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/normalize.npy'
    skip_nearest_neighbour = f'/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/domaine_creation/east_canada_squential_test_allpasses_{ANALYSED_DOMAIN}/skip_connection_UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/normalize.npy'
    topo_nearest_neighbour = f'/home/jfg000/ss5/data_superResolution/topography/nearestNeighbour_interpolation/domaine_creation/east_canada_squential_train_allpasses_{ANALYSED_DOMAIN}/ME_MG_Z0_data/normalize.npy'
    # Std deviation and mean to de-standardize the prediction
    std_inputs_de_standardize_path_nearest_neighbour = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/std.npy'
    mean_inputs_de_standardize_path_nearest_neighbour = '/home/jfg000/ss5/data_superResolution/input/domaine/nearestNeighbour_interpolation/data_used_in_neural_network_qcnb/east_canada_squential_train_allpasses_domaine/UV_UU_VV_TT_P0_PN_H_CX_SD_WGE_data/mean.npy'
    ...
    print(f'The inputs_test_no_inter is: {inputs_test_no_inter.shape}')
    skip_no_inter = np.load(skip_no_inter)
    print(f'The skip_no_inter is : {skip_no_inter.shape}')
    topo_no_inter = np.load(topo_no_inter)
    print(f'The topo_no_inter is : {topo_no_inter.shape}')

    print('load the input data for bilinear')
    inputs_test_bilinear = np.load(input_file_test_bilinear)
    print(f'The inputs_test_bilinear is: {inputs_test_bilinear.shape}')
    skip_bilinear = np.load(skip_bilinear)
    print(f'The skip_bilinear is : {skip_bilinear.shape}')
    topo_bilinear = np.load(topo_bilinear)
    print(f'The topo_bilinear is : {topo_bilinear.shape}')

    print('load the input data for nearest neighbour')
    inputs_test_nearest_neighbour = np.load(input_file_test_nearest_neighbour)
    print(f'The inputs_test_nearest_neighbour is: {inputs_test_nearest_neighbour.shape}')
    skip_nearest_neighbour = np.load(skip_nearest_neighbour)
    print(f'The skip_nearest_neighbour is : {skip_nearest_neighbour.shape}')
    topo_nearest_neighbour = np.load(topo_nearest_neighbour)
    print(f'The topo_nearest_neighbour is : {topo_nearest_neighbour.shape}')
    ...
    skip_bilinear = skip_bilinear.transpose((0,2,3,1))
    skip_nearest_neighbour = skip_nearest_neighbour.transpose((0,2,3,1))
    ...
    topo_bilinear = topo_bilinear.transpose((0,2,3,1))
    topo_nearest_neighbour = topo_nearest_neighbour.transpose((0,2,3,1))
    ...
    topo_bilinear = np.tile(topo_bilinear, (inputs_test_bilinear.shape[0], 1, 1, 1))
    print(f'The shape of inputs_test_bilinear is: {inputs_test_bilinear.shape}')
    print(f'The shape of topo_train_reshaped is : {topo_bilinear.shape}')
    # Nearest Neighbour interpolation
    topo_nearest_neighbour = np.tile(topo_nearest_neighbour, (inputs_test_nearest_neighbour.shape[0], 1, 1, 1))
    print(f'The shape of inputs_test_nearest_neighbour is: {inputs_test_nearest_neighbour.shape}')
    print(f'The shape of topo_train_reshaped is : {topo_nearest_neighbour.shape}')
    ...
    predictions_bilinear = model_bilinear.predict(
        x=[inputs_test_bilinear, topo_bilinear, skip_bilinear],
        batch_size=None,
        verbose=1,
        steps=None,
        callbacks=None,
    ...
    predictions_nearest_neighbour = model_nearest_neighbour.predict(
        x=[inputs_test_nearest_neighbour, topo_nearest_neighbour, skip_nearest_neighbour],
        batch_size=None,
        verbose=1,
        steps=None,
        callbacks=None,
    ...
    power_spectral_density_graph_average_log(predictions_no_inter_de_standardized, predictions_bilinear_de_standardized, predictions_nearest_neighbour_de_standardized, label_test_de_standardized, HP_RESULT_PATH, bm_bilinear_de_standardized)
    ...
    average_pdf_log(predictions_no_inter_de_standardized, predictions_bilinear_de_standardized, predictions_nearest_neighbour_de_standardized, label_test_de_standardized, HP_RESULT_PATH, bm_bilinear_de_standardized)
    combine_graphs(predictions_no_inter_de_standardized, predictions_bilinear_de_standardized, predictions_nearest_neighbour_de_standardized, label_test_de_standardized, HP_RESULT_PATH, bm_bilinear_de_standardized)
    average_error_location(predictions_no_inter_de_standardized, predictions_bilinear_de_standardized, predictions_nearest_neighbour_de_standardized, label_test_de_standardized, bm_bilinear_de_standardized, HP_RESULT_PATH)
    ...
    max_MAE_ni = np.max(MAE_ni)
    min_MAE_ni = np.min(MAE_ni)
    ...
    max_MSE_ni = np.max(MSE_ni)
    min_MSE_ni = np.min(MSE_ni)
    ...
    max_RMSE_ni = np.max(RMSE_ni)
    min_RMSE_ni = np.min(RMSE_ni)
    ...
    max_PSNR_ni = np.max(PSNR_ni)
    min_PSNR_ni = np.min(PSNR_ni)
    ...
    max_SSIM_ni = np.max(SSIM_ni)
    min_SSIM_ni = np.min(SSIM_ni)
    ...
    max_MAE_b = np.max(MAE_b)
    min_MAE_b = np.min(MAE_b)
    ...
    max_MSE_b = np.max(MSE_b)
    min_MSE_b = np.min(MSE_b)
    ...
    max_RMSE_b = np.max(RMSE_b)
    min_RMSE_b = np.min(RMSE_b)
    ...
    max_PSNR_b = np.max(PSNR_b)
    min_PSNR_b = np.min(PSNR_b)
    ...
    max_SSIM_b = np.max(SSIM_b)
    min_SSIM_b = np.min(SSIM_b)
    ...
    max_MAE_nn = np.max(MAE_nn)
    min_MAE_nn = np.min(MAE_nn)
    ...
    max_MSE_nn = np.max(MSE_nn)
    min_MSE_nn = np.min(MSE_nn)
    ...
    max_RMSE_nn = np.max(RMSE_nn)
    min_RMSE_nn = np.min(RMSE_nn)
    ...
    max_PSNR_nn = np.max(PSNR_nn)
    min_PSNR_nn = np.min(PSNR_nn)
    ...
    max_SSIM_nn = np.max(SSIM_nn)
    min_SSIM_nn = np.min(SSIM_nn)
    ...
    max_MAE_benchmark = np.max(MAE_benchmark)
    min_MAE_benchmark = np.min(MAE_benchmark)
    ...
    max_RMSE_benchmark = np.max(RMSE_benchmark)
    min_RMSE_benchmark = np.min(RMSE_benchmark)
    ...
    max_SSIM_benchmark = np.max(SSIM_benchmark)
    min_SSIM_benchmark = np.min(SSIM_benchmark)

    #Plot the graph for the mean metric values
    fig, axes = plt.subplots(1, 4, figsize=(30, 6))
    fig.suptitle("Average Metrics Results", fontsize=20)

    titles = ["No Interpolation", "Bilinear", "Nearest Neighbour", "Baseline"]
    cmaps = [None] 
